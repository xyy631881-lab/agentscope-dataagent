 ...
 */
package io.agentscope.dataagent.capability.marketplace.api;
import io.agentscope.dataagent.agent.api.AgentSkillsController;
import io.agentscope.dataagent.capability.marketplace.application.UserMarketplacePersistence;
...
import io.agentscope.dataagent.capability.marketplace.domain.MarketSkillSummary;
 ...
 * （通过 {@link UserMarketplacePersistence}），加上通过 {@link UserMarketplaceRegistry}
 * 浏览每个 marketplace 公开的技能的只读功能。
 *
 * <p>Marketplace 是用户私有的——两个用户可以重用相同的 ID 而不会冲突，也永远不会
 * 看到对方的条目。路径挂载在 {@code /api/me/marketplaces} 下，因此 JWT
 * 主体成为命名空间键。每个 Agent 的安装位于 {@link AgentSkillsController}，
 * 它针对 Agent 的 {@code workspace/skills/}。
 */
@RestController
