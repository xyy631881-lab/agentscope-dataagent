# 使用与测试指南（DataAgent — 前端交互版）

> 本文档面向**使用者 / 测试人员**：按功能模块逐一说明「功能在哪、怎么在页面上操作、预期看到什么」。
> 所有步骤均在**浏览器前端页面**完成，不需要 curl 或命令行工具。
>
> 想看架构与代码协作，见 [project-overview.md](file:///e:/demo/agentscope-dataagent/docs/project-overview.md) 与 [聊天链路协作详解.md](file:///e:/demo/agentscope-dataagent/docs/聊天链路协作详解.md)。
>
