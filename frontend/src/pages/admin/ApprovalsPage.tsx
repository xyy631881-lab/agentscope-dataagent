...
function DetailView({
  ...
  if (!detail) return <div style={S.sub}>正在加载内容…</div>;
    ...
    <div style={S.detail}>
      ...
      <div style={S.editRow}>
        ...
        {Object.keys(edits).length > 0 && (
          <span style={S.meta}>
            {Object.keys(edits).filter(k => isEdited(Number(k))).length} 个文件已编辑
          </span>
        )}
      </div>
...
export default function ApprovalsPage() {
  ...
  return (
    <div style={S.page}>
        ...
        <div style={S.sub}>
          批准后将在每个租户的 <code>shared/agents/&lt;agentId&gt;/</code> 下生效。
        </div>
        </div>
