  ...
  return (
    <div style={{ padding: '28px' }}>
        ...
        {data && (
          <div style={S.grid4}>
            <KpiCard
              icon="🗂" label="活跃会话" value={data.activeSessionCount}
              sub="点击浏览"
              onClick={() => navigate('/admin/sessions')}
            />
            <KpiCard
              icon="⚡" label="在线实例" value={instances.length}
              sub="点击管理"
              onClick={() => navigate('/admin/instances')}
            />
            <KpiCard
              icon="🤖" label="全局 Agents" value={data.registeredAgentCount}
              sub="点击配置"
              onClick={() => navigate('/admin/agents')}
            />
            <KpiCard
              icon="👥" label="用户" value={data.totalUserCount}
              sub="点击管理"
        ...
        {data && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 18, marginBottom: 28 }}>
            <KpiCard
              icon="📡" label="通道" value={data.registeredChannelCount}
              sub="点击查看绑定"
        ...
        <div style={S.grid2}>
          ...
          <div style={S.section}>
            <div style={S.sectionHeader}>
              <span style={S.sectionTitle}>🤖 已注册 Agents</span>
              <button style={S.seeAll} onClick={() => navigate('/admin/instances')}>查看全部 →</button>
            </div>
            <table style={S.table}>
              <thead>
                <tr>
          ...
          <div style={S.section}>
            <div style={S.sectionHeader}>
              ...
              <button style={S.seeAll} onClick={() => navigate('/admin/sessions')}>查看全部 →</button>
            </div>
            <table style={S.table}>
              <thead>
                <tr>
