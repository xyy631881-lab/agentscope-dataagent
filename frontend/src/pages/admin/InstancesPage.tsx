  ...
  return (
    <>
      <AdminPageLayout>
        {/* Toolbar */}
        <div style={S.toolbar}>
          <h2 style={S.title}>已注册 Agents</h2>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <span style={{ fontSize: '0.82rem', color: '#64748b', marginRight: 4, fontWeight: 500 }}>排序:</span>
            {(['lastActivity', 'sessions', 'agentId'] as SortKey[]).map(k => (
              <button key={k} style={{ ...S.sortBtnBase, background: sortKey === k ? '#eef2ff' : '#ffffff', border: `1px solid ${sortKey === k ? '#c7d2fe' : '#d1d5db'}`, color: sortKey === k ? '#4338ca' : '#475569' }} onClick={() => setSortKey(k)}>
                {k === 'lastActivity' ? '最近活跃' : k === 'sessions' ? '最繁忙' : 'Agent'}
              </button>
            ))}
          </div>

          <span style={S.stat}>
            {displayed.length} / {rows.length} 个 agent · {totalLiveSessions} 个活跃会话
          </span>
        </div>

        <div style={S.hint}>
          每个在 <code style={{ background: '#f1f5f9', padding: '1px 6px', borderRadius: 4, fontSize: '0.82rem' }}>agentscope.json</code> 中声明的 agentId 对应一行。
          活跃会话数和最后活动时间来自活动会话注册表。
        </div>
            ...
            {displayed.length === 0 && !loading && (
              <tr>
                <td style={{ ...S.td, color: '#94a3b8' }} colSpan={5}>
                  {rows.length === 0 ? '没有已注册的 agent。' : '没有匹配当前筛选条件的 agent。'}
                </td>
              </tr>
            )}
            {displayed.map(r => (
