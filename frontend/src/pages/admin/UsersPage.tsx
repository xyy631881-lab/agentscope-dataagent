...
export default function UsersPage() {
  ...
  const tabs = [
    { key: 'list', label: '用户', icon: '👥', badge: loading ? '…' : users.length },
    { key: 'add',  label: '添加用户', icon: '＋' },
  ];
    ...
    <>
      <AdminPageLayout tabs={tabs} activeTab={tab} onTabChange={k => setTab(k as Tab)}>
        ...
        {tab === 'list' && (
          <>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
              <button style={S.refreshBtn} onClick={load} disabled={loading}>{loading ? '…' : '↺ 刷新'}</button>
            </div>
            <table style={S.table}>
              <thead>
                <tr>
            ...
            {!loading && users.length === 0 && <p style={{ color: '#94a3b8', fontSize: '0.92rem', marginTop: 16 }}>未找到用户。</p>}
          </>
        )}
        ...
        {tab === 'add' && (
          <div style={S.formWrap}>
            <div style={S.formCard}>
              ...
              <input style={S.input} value={newUsername} onChange={e => setNewUsername(e.target.value)} placeholder="用户名" autoFocus />
              <label style={S.label}>Password</label>
              <input style={S.input} type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="至少 6 个字符" />
        ...
        {modalMode === 'edit-roles' && targetUser && (
          <div style={S.modal} onClick={() => setModalMode(null)}>
            <div style={S.modalBox} onClick={e => e.stopPropagation()}>
              <h3 style={S.modalTitle}>编辑角色 — {targetUser.username}</h3>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
                <input type="checkbox" id="editAdmin" checked={editAdmin} onChange={e => setEditAdmin(e.target.checked)} />
                <label htmlFor="editAdmin" style={{ fontSize: '0.92rem', color: '#0f172a', fontWeight: 500 }}>管理员角色</label>
              </div>
              {formError && <div style={S.err}>{formError}</div>}
              <div style={S.modalActions}>
                <button style={S.secBtn} onClick={() => setModalMode(null)}>取消</button>
                <button style={S.primaryBtn} onClick={handleEditRoles}>保存</button>
              </div>
            </div>
          </div>
        )}
        ...
        {modalMode === 'reset-password' && targetUser && (
          <div style={S.modal} onClick={() => setModalMode(null)}>
            <div style={S.modalBox} onClick={e => e.stopPropagation()}>
              <h3 style={S.modalTitle}>重置密码 — {targetUser.username}</h3>
              <label style={{ ...S.label, marginBottom: 6 }}>新密码</label>
              <input style={S.modalInput} type="password" value={resetPassword} onChange={e => setResetPassword(e.target.value)} placeholder="至少 6 个字符" />
              {formError && <div style={S.err}>{formError}</div>}
              <div style={S.modalActions}>
                <button style={S.secBtn} onClick={() => setModalMode(null)}>取消</button>
                <button style={S.primaryBtn} onClick={handleResetPassword}>重置</button>
              </div>
            </div>
          </div>
        )}
