...
export default function SubagentPanel({ agentId, onChanged }: Props) {
      ...
      <div style={S.root}>
        <div style={S.overlay} onClick={() => setView('list')}>
          <div style={S.dialog} onClick={e => e.stopPropagation()}>
            <div style={S.dialogTitle}>从现有 agent 添加子 Agent</div>
            <input
              style={S.searchInput}
  ...
  if (view === 'edit') {
    return (
      <div style={S.root}>
        <div style={S.header}>
          <button style={S.btn} onClick={() => setView('list')}>← 返回</button>
          <span style={S.title}>{isNew ? '新建子 Agent' : `编辑: ${form.name}`}</span>
        </div>
        <div style={S.scroll}>
          <div style={S.row}>
          ...
          <div style={S.row}>
            ...
            <input
              ...
              placeholder="此子 Agent 的功能 — 编排器根据此描述决定何时委派任务"
            />
          </div>
          <div style={{ ...S.row, ...S.inlineRow }}>
            <div style={S.inlineField}>
              ...
              <input
                ...
                placeholder="例如 qwen3-max（留空则继承父级）"
              />
            </div>
            <div style={S.inlineField}>
          ...
          <div style={S.row}>
            ...
            <input
              ...
              placeholder="例如 read_file, grep_files, edit_file"
            />
          </div>
          <div style={S.row}>
            <label style={S.fieldLabel}>工作区模式</label>
            <div style={S.radio}>
          ...
          {form.workspaceMode === 'isolated' && (
            <div style={S.row}>
              <label style={S.fieldLabel}>工作区路径（可选）</label>
              <input
                style={S.input}
                value={form.workspacePath}
                onChange={e => setForm({ ...form, workspacePath: e.target.value })}
                placeholder="例如 ./defs/reviewer（留空则自动创建）"
              />
            </div>
          )}
          <div style={S.row}>
            <label style={S.fieldLabel}>系统提示词（内联内容）</label>
            <textarea
              style={S.textarea}
              value={form.inlineBody}
              onChange={e => setForm({ ...form, inlineBody: e.target.value })}
              placeholder="此子 Agent 的可选系统提示词。设置工作区路径时不使用。"
            />
          </div>
  ...
  return (
    <div style={S.root}>
      <div style={S.header}>
        <span style={S.title}>子 Agents</span>
        <button style={S.btn} onClick={openPicker}>+ 从 agent 添加</button>
        <button style={S.primaryBtn} onClick={handleNew}>+ 新建</button>
      </div>
      <div style={S.scroll}>
